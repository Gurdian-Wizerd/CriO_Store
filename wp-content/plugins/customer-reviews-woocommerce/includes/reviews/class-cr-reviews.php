	...
	public function show_summary_table( $product_id, $is_ajax = false, $new_reviews_allowed = false ) {
		...
		if( $all > 0 ) {
			...
			if ('yes' !== get_option('ivole_reviews_nobranding', 'yes')) {
				...
				$output .= '<span>Powered by</span><a href="https://wordpress.org/plugins/customer-reviews-woocommerce/" target="_blank" alt="Customer Reviews for WooCommerce" title="Customer Reviews for WooCommerce"><img src="' . plugins_url( '/img/logo-vs.svg', dirname( dirname( __FILE__ ) ) ) . '"></a>';
				$output .= '</div>';
			}
