...
class arfliteformhelper {
	...
	function arflite_get_default_options() {
		...
		return array(
			'edit_value'                 => $arflite_style_settings->update_value,
			'edit_msg'                   => $arflite_style_settings->edit_msg,
			'logged_in_role'             => '',
			'editable_role'              => '',
			'open_editable'              => 0,
			'open_editable_role'         => '',
			...
			'form_custom_css'            => '',
			'label_position'             => $arflite_style_settings->position,
			'is_custom_css'              => 0,
			...
			'email_to'                   => $arflitesettings->reply_to,
			'reply_to'                   => $arflitesettings->reply_to,
			...
			'ar_user_from_name'          => ( isset( $arflitesettings->ar_user_from_name ) ) ? $arflitesettings->ar_user_from_name : '',
			'ar_user_from_email'         => ( isset( $arflitesettings->ar_user_from_email ) ) ? $arflitesettings->ar_user_from_email : '',
			'ar_user_nreplyto_email'     => ( isset( $arflitesettings->ar_user_nreplyto_email ) ) ? $arflitesettings->ar_user_nreplyto_email : '',
			'ar_admin_from_name'         => ( isset( $arflitesettings->ar_admin_from_name ) ) ? $arflitesettings->ar_admin_from_name : '',
			'ar_admin_from_email'        => ( isset( $arflitesettings->ar_admin_from_email ) ) ? $arflitesettings->ar_admin_from_email : '',
			'admin_nreplyto_email'       => ( isset( $arflitesettings->admin_nreplyto_email ) ) ? $arflitesettings->admin_nreplyto_email : '',
			...
			'arf_pre_dup_field'          => '',
			'arf_pre_dup_msg'            => $arflitesettings->arf_pre_dup_msg,
	...
	function arflite_setup_new_vars() {
		...
		global $ARFLiteMdlDb, $arflitesettings, $arfliteformhelper, $arflitemainhelper;
		...
		$values['form_key'] = ( $_POST && isset( $_POST['form_key'] ) ) ? sanitize_text_field( $_POST['form_key'] ) : ( $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->forms, 'form_key' ) ); //phpcs:ignore
		...
		$values['custom_style'] = ( isset( $_POST ) && $_POST && isset( $_POST['options']['custom_style'] ) ) ? sanitize_text_field( $_POST['options']['custom_style'] ) : ( $arflitesettings->load_style != 'none' ); //phpcs:ignore
	...
	function arflite_get_default_opts() {
		...
		return array(
			...
			'submit_value'           => $arflitesettings->submit_value,
			'success_action'         => 'message',
			'success_msg'            => $arflitesettings->success_msg,
			'show_form'              => 0,
			'akismet'                => '',
			'ar_email_message'       => __( 'Thank you for subscription with us. We will contact you soon.', 'arforms-form-builder' ),
			'ar_admin_email_message' => '[ARFLite_form_all_values]',
	...
	function arflite_forms_dropdown( $field_name, $field_value = '', $blank = true, $field_id = false, $onchange = false ) {
		...
		if ( false === $record_count ) {
			$record_count = $wpdb->get_results( "SELECT $ARFLiteMdlDb->forms.id, COUNT($ARFLiteMdlDb->entries.id) AS count_num FROM $ARFLiteMdlDb->entries RIGHT JOIN $ARFLiteMdlDb->forms ON $ARFLiteMdlDb->entries.form_id=$ARFLiteMdlDb->forms.id WHERE $ARFLiteMdlDb->forms.is_template=0 AND ($ARFLiteMdlDb->forms.status is NULL OR $ARFLiteMdlDb->forms.status = '' OR $ARFLiteMdlDb->forms.status = 'published') group by $ARFLiteMdlDb->forms.id", OBJECT_K ); //phpcs:ignore
