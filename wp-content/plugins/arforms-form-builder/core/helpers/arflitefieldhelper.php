...
class arflitefieldhelper {
	...
	function arflite_setup_new_vars( $values, $field ) {
		...
		if ( $values['type'] == 'date' ) {
			if ( preg_match( '/^\d{4}-\d{2}-\d{2}$/', $values['value'] ) ) {
				...
				$values['value'] = $arflitemainhelper->arfliteconvert_date( $values['value'], 'Y-m-d', $arflite_style_settings->date_format );
			}
	...
	function arflite_setup_new_variables( $type = '', $form_id = '' ) {
		global $arflitesettings, $arflitefieldhelper;
		...
		if ( $type == 'checkbox' ) {
			...
			$values['invalid'] = $arflitesettings->re_msg;
		}
	...
	function arflite_get_linked_options( $values, $field, $entry_id = false ) {
		global $arfliterecordmeta, $user_ID, $arflitefield, $ARFLiteMdlDb, $arfliterecordhelper;
		$metas          = array();
		...
		if ( $values['restrict'] && $user_ID ) {
			...
			if ( $entry_id && is_admin() ) {
				$entry_user = $ARFLiteMdlDb->arfliteget_var( $ARFLiteMdlDb->entries, array( 'id' => $entry_id ), 'user_id' );
			...
			if ( isset( $selected_field->form_id ) ) {
				...
				$entry_ids    = $ARFLiteMdlDb->arfliteget_col( $ARFLiteMdlDb->entries, $linked_where, 'id' );
				unset( $linked_where );
			}
			...
			$metas      = $ARFLiteMdlDb->arflite_get_records( $ARFLiteMdlDb->entry_metas, array( 'field_id' => $values['form_select'] ), 'entry_value', '', 'entry_id, entry_value' );
			$attach_ids = $ARFLiteMdlDb->arflite_get_records( $ARFLiteMdlDb->entries, array( 'form_id' => $selected_field->form_id ), '', '', 'id, attachment_id' );
		}
	...
	function arflite_get_default_field_options( $type, $field, $limit = false ) {
		...
		$field_options = array(
			...
			'max'                        => '',
			'label'                      => '',
			'blank'                      => addslashes( $arflitesettings->blank_msg ),
			'max_rows'                   => '3',
			'required_indicator'         => '*',
			'invalid'                    => '',
			'separate_value'             => 0,
		...
		global $ARFLiteMdlDb, $arflitemainhelper, $arflitesettings;

		$form_id = ( is_numeric( $field ) ) ? $field : $field->form_id;

		$key         = is_numeric( $field ) ? $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->fields, 'field_key' ) : $field->field_key;
		$field_count = $arflitemainhelper->arflitegetRecordCount( "form_id='$form_id'", $ARFLiteMdlDb->fields );
		...
		return array(
			...
			'default_value' => '',
			'required'      => false,
			'blank'         => $arflitesettings->blank_msg,
			'unique_msg'    => $arflitesettings->unique_msg,
	...
	function arfliteget_date_entry( $value, $form_id, $show_time_calendar = false, $field_clock = 12, $locale = 'en', $from_addon = '' ) {
		...
		if ( false == $form_data ) {
			$form_data = $wpdb->get_results( $wpdb->prepare( 'SELECT form_css FROM ' . $ARFLiteMdlDb->forms . ' WHERE id=%d', $form_id ) ); //phpcs:ignore
	...
	function arflite_get_field_type( $filed_id = '' ) {
		...
		global $wpdb, $ARFLiteMdlDb;
		$res = $wpdb->get_results( $wpdb->prepare( 'SELECT id, type FROM ' . $ARFLiteMdlDb->fields . ' WHERE id = %d', $filed_id ) ); //phpcs:ignore
	...
	function arflite_get_confirm_email_field( $field ) {
		...
		global $ARFLiteMdlDb, $wpdb, $arflitemainhelper, $arflitefieldhelper;

		$key     = $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->fields, 'field_key' );
		...
		if ( $frm_css['arfinputstyle'] == 'material' ) {
			...
			if ( $frm_css['arfinputstyle'] == 'material' ) {
				...
				<span class="arfeditorfieldopt_label arf_edit_in_place">
					<input type="text" class="arf_edit_in_place_input inplace_field" data-ajax="false" data-field-opt-change="true" data-field-opt-key='name' value="<?php echo htmlspecialchars( esc_attr( $field['name'] ) ); //phpcs:ignore ?>" data-field-id="<?php echo esc_attr( $field['id'] ); ?>" />
				</span>
	...
	function arflite_add_blank_msg_from_globalsetting( $form_field_json, $arfinputstyle ) {
		...
		foreach ( $form_field_json->field_data as $key => $value ) {
			foreach ( $value as $data_key => $data_value ) {
				if ( $data_key == 'blank' ) {
					$form_field_json->field_data->$key->$data_key = $arflitesettings->blank_msg;
				}
			}
		}
		$default_data_varible                                        = 'default_data_' . $arfinputstyle;
		$form_field_json->$default_data_varible->arfsubmitbuttontext = $arflitesettings->submit_value;
