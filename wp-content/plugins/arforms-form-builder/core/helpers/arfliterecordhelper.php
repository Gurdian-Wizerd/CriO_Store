...
class arfliterecordhelper {
	...
	function arflite_setup_new_vars( $fields, $form = '', $reset = false ) {
		...
		global $arfliteform, $arflitesettings, $arflitesidebar_width, $arflitefieldhelper, $arflitemainhelper, $arfliteformhelper;
		...
		if ( $fields ) {
			...
			if ( ! isset( $values['custom_style'] ) ) {
				$values['custom_style'] = ( $arflitesettings->load_style != 'none' );
			}
			...
			if ( ! isset( $values['submit_value'] ) ) {
				$values['submit_value'] = $arflitesettings->submit_value;
			}

			if ( ! isset( $values['success_msg'] ) ) {
				$values['success_msg'] = $arflitesettings->success_msg;
			}
	...
	function arflitedisplay_value( $value, $field, $atts = array(), $form_css = array(), $incomplete_entry = false, $form_name = '' ) {
		...
		global $wpdb, $arflitefieldhelper, $arflitemainhelper, $ARFLiteMdlDb;

		$entry_table      = $ARFLiteMdlDb->entries;
		$entry_meta_table = $ARFLiteMdlDb->entry_metas;
		if ( true == $incomplete_entry ) {
			$entry_table      = $ARFLiteMdlDb->incomplete_entries;
			$entry_meta_table = $ARFLiteMdlDb->incomplete_entry_metas;
		}
		$defaults = array(
			'type'          => '',
			'show_icon'     => true,
