...
class arfliterecordmodel {
	...
	function arflitecreate( $values, $return_values = false ) {
		global $wpdb, $ARFLiteMdlDb, $arfliterecordmeta, $arflite_fid, $arflitemainhelper, $arflite_db_record, $arflitefieldhelper, $arflitesettings, $arfliteformhelper, $arflitecreatedentry;
		...
		if ( ! $form_options ) {
			$form_options = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $form_id ) ); //phpcs:disable
		...
		$new_values['entry_key'] = $arflitemainhelper->arflite_get_unique_key( $values['entry_key'], $ARFLiteMdlDb->entries, 'entry_key' );
		...
		if ( $create_entry ) {
			$query_results = $wpdb->insert( $ARFLiteMdlDb->entries, $new_values );
		}
		...
		if ( isset( $query_results ) && $query_results ) {
			...
			if ( isset( $_REQUEST['form_display_type'] ) && $_REQUEST['form_display_type'] != '' ) {
				...
				$wpdb->insert( $ARFLiteMdlDb->entry_metas, $arf_meta_insert, array( '%s', '%d', '%d', '%s' ) );
			}
	...
	function &arflitedestroy( $id ) {
		...
		$wpdb->query( $wpdb->prepare( 'DELETE FROM ' . $ARFLiteMdlDb->entry_metas . ' WHERE entry_id=%d', $id ) ); //phpcs:ignore

		$result = $wpdb->query( $wpdb->prepare( 'DELETE FROM ' . $ARFLiteMdlDb->entries . ' WHERE id=%d', $id ) ); //phpcs:ignore
	...
	function arflitegetOne( $id, $meta = false ) {
		...
		$query = "SELECT it.*, fr.name as form_name, fr.form_key as form_key FROM $ARFLiteMdlDb->entries it


                  LEFT OUTER JOIN $ARFLiteMdlDb->forms fr ON it.form_id=fr.id WHERE ";
	...
	function arflitegetAll( $where = '', $order_by = '', $limit = '', $meta = false, $inc_form = true, $arfSearch = '', $arffieldorder = array() ) {
		...
		$entry_meta_table = $ARFLiteMdlDb->entry_metas;
		...
		if ( $inc_form ) {
			...
			$query = "SELECT it.*, fr.name as form_name,fr.form_key as form_key
                ...
                FROM $entry_table it LEFT OUTER JOIN $ARFLiteMdlDb->forms fr ON it.form_id=fr.id" . $left_outer_join .
		...
		if ( $meta && $entries ) {
			...
			$query = $wpdb->prepare( "SELECT entry_id, entry_value, field_id, fi.field_key as field_key FROM $entry_meta_table it LEFT OUTER JOIN $ARFLiteMdlDb->fields fi ON it.field_id=fi.id WHERE $meta_where and field_id != %d", 0 ); //phpcs:ignore
	...
	function arflitegetRecordCount( $where = '', $entry2 = false ) {
		...
		if ( is_numeric( $where ) ) {
			...
			$query = "SELECT COUNT(*) FROM $entry_table it LEFT OUTER JOIN $ARFLiteMdlDb->forms fr ON it.form_id=fr.id" .
