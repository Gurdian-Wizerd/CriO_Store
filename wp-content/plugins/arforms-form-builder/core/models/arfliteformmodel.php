...
class arfliteformmodel {
	...
	function arflitecreate( $values ) {
		global $wpdb, $ARFLiteMdlDb, $arflitesettings, $arfliteformhelper, $arflitemainhelper;
		$new_values = array();
		if ( $values['form_key'] == '' ) {
			$new_values['form_key'] = $arflitemainhelper->arflite_get_unique_key( $values['form_key'], $ARFLiteMdlDb->forms, 'form_key' );
		} else {
		...
		$query_results = $wpdb->insert( $ARFLiteMdlDb->forms, $new_values );
	...
	function arfliteduplicate( $id, $template = false, $copy_keys = false, $blog_id = false, $is_from_edit = false, $newformid = 0, $is_ref_form = 0 ) {
		global $wpdb, $ARFLiteMdlDb, $arfliteform, $arflitefield, $arfliteformhelper, $arflitemainhelper;
		...
		$new_values['form_key']    = $arflitemainhelper->arflite_get_unique_key( $new_key, $ARFLiteMdlDb->forms, 'form_key' );
		...
		if ( $newformid > 0 ) {
			$query_results = $wpdb->update( $ARFLiteMdlDb->forms, $new_values, array( 'id' => $newformid ) );
		} else {
			$query_results = $wpdb->insert( $ARFLiteMdlDb->forms, $new_values );
		}
		if ( $query_results ) {
			...
			if ( $is_from_edit ) {
				...
				$form_options_sql = $wpdb->get_results( $wpdb->prepare( 'SELECT options FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $form_id ) ); //phpcs:ignore
				$form_options     = maybe_unserialize( $form_options_sql[0]->options );

				if ( $template < 100 ) {
					global $arflitesettings;
					$form_options['success_msg'] = $arflitesettings->success_msg;
				}
				...
				if ( isset( $_SESSION['arf_fields'] ) && is_array( $_SESSION['arf_fields'] ) && count( $_SESSION['arf_fields'] ) > 0 ) {
							...
							foreach ( $fields_array as $new_field ) {
								...
								if ( count( $arf_field_options ) > 0 ) {
									...
									if ( $new_field->type == 'html' ) {
										...
										$wpdb->update( $ARFLiteMdlDb->fields, array( 'description' => $newdescription ), array( 'id' => $new_field->id ) );
									}
									$new_field_options = maybe_serialize( $new_field_options );
									$wpdb->update( $ARFLiteMdlDb->fields, array( 'field_options' => $new_field_options ), array( 'id' => $new_field->id ) );
								}
							}
						}
					}
					...
					$wpdb->update( $ARFLiteMdlDb->forms, array( 'options' => $form_options_new ), array( 'id' => $form_id ) );
	...
	function arflitedestroy( $id ) {
		...
		global $wpdb, $ARFLiteMdlDb, $arflite_db_record;
		...
		$form_css_res = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM ' . $ARFLiteMdlDb->forms . ' WHERE id = %d', $id ), ARRAY_A ); //phpcs:ignore
		...
		if ( $form_css_res ) {
			foreach ( $form_css_res as $refform ) {
				...
				if ( isset( $rformid ) && $rformid > 0 && $rformid != '' ) {
					...
					$query_results_r1 = $wpdb->query( $wpdb->prepare( "DELETE FROM `$ARFLiteMdlDb->fields` WHERE `form_id` = %d", $rformid ) ); //phpcs:ignore
					...
					$query_results = $wpdb->query( $wpdb->prepare( "DELETE FROM `$ARFLiteMdlDb->forms` WHERE `id` = %d", $rformid ) ); //phpcs:ignore
				}
			}
		}
		...
		$query_results = $wpdb->query( $wpdb->prepare( "DELETE FROM `$ARFLiteMdlDb->fields` WHERE `form_id` = %d", $id ) ); //phpcs:ignore
		...
		$query_results = $wpdb->query( $wpdb->prepare( "DELETE FROM `$ARFLiteMdlDb->forms` WHERE `id` = %d", $id ) ); //phpcs:ignore
	...
	function arflitegetName( $id ) {
		...
		$query = "SELECT name FROM $ARFLiteMdlDb->forms WHERE ";
	...
	function arflitegetAll( $where = array(), $order_by = '', $limit = '', $is_ref_form = 0 ) {
		...
		$query = 'SELECT * FROM ' . $ARFLiteMdlDb->forms . $arflitemainhelper->arfliteprepend_and_or_where( ' WHERE ', $where ) . $order_by . $limit;

		if ( $limit == ' LIMIT 1' || $limit == 1 ) {

			if ( is_array( $where ) ) {
				$results = $ARFLiteMdlDb->arflite_get_one_record( $ARFLiteMdlDb->forms, $where, '*', $order_by );
			} else {
			...
			if ( is_array( $where ) ) {
				$results = $ARFLiteMdlDb->arflite_get_records( $ARFLiteMdlDb->forms, $where, $order_by, $limit );
			} else {
	...
	function arflite_has_field( $type, $form_id, $single = true ) {
		...
		if ( $single ) {
			$included = $ARFLiteMdlDb->arflite_get_one_record( $ARFLiteMdlDb->fields, compact( 'form_id', 'type' ) );
		} else {
			$included = $ARFLiteMdlDb->arflite_get_records( $ARFLiteMdlDb->fields, compact( 'form_id', 'type' ) );
		}
	...
	function arflite_post_type( $form_id ) {
		...
		if ( is_numeric( $form_id ) ) {
			...
			if ( $cache ) {
				...
				$form_options = $ARFLiteMdlDb->arfliteget_var( $ARFLiteMdlDb->forms, array( 'id' => $form_id ), 'options' );
			}
