...
class arflitefieldmodel {
	...
	function arflitecreate( $values, $return = true, $template = false, $res_field_id = '' ) {
		global $wpdb, $ARFLiteMdlDb, $arflitemainhelper,$arfliteformcontroller;
		$new_values                  = array();
		...
		$new_values['field_key']     = $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->fields, 'field_key' );
		...
		$query_results = $wpdb->insert( $ARFLiteMdlDb->fields, $new_values );
		if ( $return ) {
			if ( $query_results ) {
	...
	function arfliteduplicate( $old_form_id, $form_id, $copy_keys = false, $blog_id = false, $template = false ) {
		...
		$form_options    = $wpdb->get_results( $wpdb->prepare( 'SELECT `options` FROM `' . $ARFLiteMdlDb->forms . '` WHERE `id` = %d', $old_form_id ) ); //phpcs:ignore
	...
	function arfliteupdate( $id, $values ) {
		global $wpdb, $ARFLiteMdlDb, $arflitefieldhelper, $arflitemainhelper;
		if ( isset( $values['field_key'] ) ) {
			$values['field_key'] = $arflitemainhelper->arflite_get_unique_key( $values['field_key'], $ARFLiteMdlDb->fields, 'field_key', $id );
		}
		...
		$query_results = $wpdb->update( $ARFLiteMdlDb->fields, $values, array( 'id' => $id ) );
		unset( $values );
		if ( $query_results ) {
	...
	function arflitedestroy( $id ) {
		...
		$wpdb->query( $wpdb->prepare( "DELETE FROM $ARFLiteMdlDb->entry_metas WHERE field_id=%d", $id ) ); //phpcs:ignore
		return $wpdb->query( $wpdb->prepare( "DELETE FROM $ARFLiteMdlDb->fields WHERE id=%d", $id ) ); //phpcs:ignore
	}
	...
	function arflitegetOne( $id ) {
		...
		if ( ! $results ) {
			...
			$results = $ARFLiteMdlDb->arflite_get_one_record( $ARFLiteMdlDb->fields, $where );
			if ( $results ) {
	...
	function arflitegetAll( $where = array(), $order_by = '', $limit = '', $blog_id = false, $is_ref_form = 0 ) {
		global $wpdb, $ARFLiteMdlDb, $arflitemainhelper;
		$table_name      = $ARFLiteMdlDb->fields;
		$form_table_name = $ARFLiteMdlDb->forms;
		...
		if ( $table_name == $ARFLiteMdlDb->fields ) {
		...
		if ( $results ) {
			if ( is_array( $results ) ) {
				foreach ( $results as $r_key => $result ) {
					...
					$results[ $r_key ]->field_options['arf_regular_expression'] = isset( $results[ $r_key ]->field_options['arf_regular_expression'] ) ? $results[ $r_key ]->field_options['arf_regular_expression'] : '';
