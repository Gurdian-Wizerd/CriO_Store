...
class arflitenotifymodel {
	...
	function arfliteentry_created( $entry_id, $form_id ) {
		...
		$allfields = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM ' . $ARFLiteMdlDb->fields . ' WHERE form_id = %d order by id', $form_id ), ARRAY_A ); //phpcs:ignore
		...
		foreach ( $values as $value ) {
			...
			if ( $value->field_type == 'select' || $value->field_type == 'checkbox' || $value->field_type == 'radio' ) {
				global $wpdb,$ARFLiteMdlDb;
				$field_opts = $wpdb->get_row( $wpdb->prepare( 'SELECT entry_value FROM ' . $ARFLiteMdlDb->entry_metas . " WHERE field_id='%d' AND entry_id='%d'", '-' . $value->field_id, $entry->id ) ); //phpcs:ignore
				if ( $field_opts ) {
				...
				if ( $field_opts ) {
					...
					if ( $value->field_type == 'checkbox' ) {
						...
						if ( $value->field_type == 'select' ) {
							...
							$field_tmp      = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->fields . " WHERE id = '%d'", $field_id ) ); //phpcs:ignore
							...
							if ( $field_tmp_opts['separate_value'] ) {
								...
								$get_field_label = $wpdb->get_row( $wpdb->prepare( 'SELECT entry_value FROM ' . $ARFLiteMdlDb->entry_metas . ' WHERE field_id = "-%d" and entry_id="%d"', $label_field_id, $value->entry_id ) ); //phpcs:ignore
