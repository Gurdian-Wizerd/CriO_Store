...
global $wpdb, $arflitesettingcontroller, $arfliteformcontroller, $ARFLiteMdlDb, $arflitefieldhelper;
...
$character_set = isset( $arflitesettings->arf_css_character_set ) && ! empty( $arflitesettings->arf_css_character_set ) ? (array) $arfsettings->arf_css_character_set : array();
...
if ( empty( $is_form_save ) ) {
	...
	echo esc_html( $arf_form_cls_prefix_without_material_container ) . ' #arf_message_success .msg-detail::before{';
		echo 'background-image: url(data:image/svg+xml;base64,' . base64_encode( esc_html('<svg width="60" height="60" xmlns="http://www.w3.org/2000/svg" xmlns:xlink="http://www.w3.org/1999/xlink" version="1.1" x="0px" y="0px" viewBox="0 0 52 52" enable-background="new 0 0 52 52" xml:space="preserve"><g><path fill="' . esc_attr( $success_text_color ) . '" d="M26,0C11.66,0,0,11.66,0,26s11.66,26,26,26s26-11.66,26-26S40.34,0,26,0z M26,50C12.77,50,2,39.23,2,26   S12.77,2,26,2s24,10.77,24,24S39.23,50,26,50z"/><path fill="' . esc_attr( $success_text_color ) . '" d="M38.25,15.34L22.88,32.63l-9.26-7.41c-0.43-0.34-1.06-0.27-1.41,0.16c-0.35,0.43-0.28,1.06,0.16,1.41l10,8   C22.56,34.93,22.78,35,23,35c0.28,0,0.55-0.11,0.75-0.34l16-18c0.37-0.41,0.33-1.04-0.08-1.41C39.25,14.88,38.62,14.92,38.25,15.34   z"/></g></svg>' ) ) . ');'; //phpcs:ignore
	...
	if ( in_array( 'checkbox', $loaded_field ) ) {
		...
		if ( $is_checkbox_img_enable ) {
			...
			if ( $arflite_preview ) {
				...
				$all_checkbox_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT id,field_options FROM ' . $ARFLiteMdlDb->fields . ' WHERE type = %s AND form_id = %s', 'checkbox', $form_id ) ); //phpcs:ignore
			}
	...
	if ( in_array( 'radio', $loaded_field ) ) {
		...
		if ( $is_radio_img_enable ) {
			...
			if ( $arflite_preview ) {
				...
				$all_radio_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT id,field_options FROM ' . $ARFLiteMdlDb->fields . ' WHERE type = %s AND form_id = %s', 'radio', $form_id ) ); //phpcs:ignore
			}
