<?php
	...
	$arf_import_export_useragent = isset( $_SERVER['HTTP_USER_AGENT'] ) ? $_SERVER['HTTP_USER_AGENT'] : '';
...
<div class="wrap arfforms_page arf_imortexport arf_imortexport_page_wrap">
			...
			<div class="inside">
				...
				<input type="hidden" name="arflite_validation_nonce" id="arflite_validation_nonce" value="<?php echo esc_attr( wp_create_nonce( 'arflite_wp_nonce' ) ); ?>" />
					...
					if ( isset( $_REQUEST['arf_import_btn'] ) && current_user_can( 'arfchangesettings' ) ) {
						...
						if( !isset( $_REQUEST['arflite_import_form_nonce'] ) || ( isset(  $_REQUEST['arflite_import_form_nonce'] ) && !wp_verify_nonce( $_REQUEST['arflite_import_form_nonce'], 'arflite_import_form' ) ) ){ //phpcs:ignore
									...
									foreach ( $formxml->children() as $key_main => $val_main ) {
										...
										foreach ( $new_field_order as $key => $value ) {
											if ( strpos( $key, '_confirm' ) !== false ) {
												...
												$fleld_data_confirm                        = $wpdb->get_results( $wpdb->prepare( 'SELECT field_options FROM ' . $ARFLiteMdlDb->fields . ' WHERE id=%d', $new_id ) );//phpcs:ignore
										...
										$getForm = $wpdb->get_results( $wpdb->prepare( 'SELECT options FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $form_id ) ); //phpcs:ignore
										...
										if ( ! empty( $new_values ) ) {
											$query_results = $wpdb->query( $wpdb->prepare( 'update ' . $ARFLiteMdlDb->forms . " set form_css = '%s' where id = '%d'", $cssoptions_db, $form_id ) ); //phpcs:ignore 
										...
										$sel_rec = $wpdb->prepare( 'select options from ' . $ARFLiteMdlDb->forms . ' where id = %d', $form_id ); //phpcs:ignore
											...
											foreach ( $val_main->form_entries->children() as $key_fields => $val_fields ) {
												...
												foreach ( $val_fields as $key_field => $val_field ) {
													...
													if ( $field_nm == 'Browser' ) {
														...
														$field_data = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->fields . ' WHERE form_id = %d', $frm_id ) ); //phpcs:ignore
					...
					<div class="modal-body arfexportformwrap">
						...
						<div class="export_opt_part" id="export_opt_part">
							...
							<form id="exportForm" onSubmit="return arflite_check_import_form_selected();" method="post">
								<input type="hidden" value="<?php echo esc_url(site_url() . '/index.php?plugin=ARFormslite'); ?>" name="arflitescripturl_cus" id="arflitescripturl_cus" />
								<div id="export_forms" class="export_forms" >
									<div class="export_options" id="export_options">
										<div class="arf_radio_wrapper">
											...
											<span>
												<label for="arflite_opt_export_form"><?php echo esc_html__( 'Form(s) Only', 'arforms-form-builder' ); ?></label>
											</span>
										</div>
										<div class="arf_radio_wrapper">
											<div class="arf_custom_radio_div" >
											...
											<span>
												<label for="arflite_opt_export_entries"><?php echo esc_html__( 'Entries Only', 'arforms-form-builder' ); ?></label>
											</span>
										</div>
										<div class="arf_radio_wrapper" style="<?php echo ( ! is_rtl() ) ? 'width:60%;' : ''; ?>">
											...
											<span>
												<label for="arflite_opt_export_both"><?php echo esc_html__( 'Forms + Entries', 'arforms-form-builder' ); ?></label>
											</span>
										</div>
									</div>
								...
								<table class="form-table">
									...
									<tr>
										<td class="arfimportbtntd">
											<input type="hidden" name="arf_xml_file_name" id="arf_xml_file_name" value="" /><input type="hidden" name="arf_import_disable" id="arf_import_disable" value="1" /><input type="hidden" name="arflite_import_form_nonce" value="<?php echo esc_attr(wp_create_nonce( 'arflite_import_form' )); ?>" />
	...
	<div class="documentation_link arflite-clear-float" align="right"><a href="https://www.arformsplugin.com/documentation/1-getting-started-with-arforms/" target="_blank" class="arlinks doc-link-a">
			<?php echo esc_html__( 'Documentation', 'arforms-form-builder' ); ?>
		</a>
	</div>
</div>
