...
<html <?php language_attributes(); ?>>
	<head>
		...
		echo stripslashes_deep( get_option( 'arf_global_css' ) ); //phpcs:ignore
	...
	<body id="arf_preview_body" class="arf_preview_modal_body <?php echo ( is_rtl() ) ? 'arf_preview_rtl' : ''; ?>" >
		...
		if ( isset( $form->id ) && ! isset( $_REQUEST['form_id'] ) ) {
			$res = $wpdb->get_results( $wpdb->prepare( 'SELECT options FROM ' . $ARFLiteMdlDb->forms . ' WHERE id = %d', $form->id ), 'ARRAY_A' ); //phpcs:ignore
		}
		if ( isset( $res ) ) {
			$res = $res[0];
		}
