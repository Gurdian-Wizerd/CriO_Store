...
foreach ( $entries as $entry ) {
	...
	$res_data    = $wpdb->get_results( $wpdb->prepare( 'SELECT description,country, browser_info FROM ' . $ARFLiteMdlDb->entries . ' WHERE id = %d', $entry->id ), 'ARRAY_A' ); //phpcs:ignore
	$description = maybe_unserialize( $res_data[0]['description'] );
	/* changes http_page_url start */
	$arflite_page_url = $wpdb->get_row( $wpdb->prepare( 'SELECT entry_value FROM ' . $ARFLiteMdlDb->entry_metas . " WHERE field_id='%d' AND entry_id='%d'", '-' . 0, $entry->id ) ); //phpcs:ignore
	if ( ! empty( $arflite_page_url ) ) {
		...
		if ( ! empty( $http_referrer_url ) ) {
			...
			$entry->page_url = isset( $http_page_url[1] ) ? $http_page_url[1] : '';
		}
	} else {
