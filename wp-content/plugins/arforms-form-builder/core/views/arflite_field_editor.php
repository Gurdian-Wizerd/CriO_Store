...
if ( isset( $field_resize_width[ $arf_field_counter ] ) ) {
	$sortable_inner_field_style = "style='width: " . esc_attr( $field_resize_width[ $arf_field_counter ] ) . "%' data-width='" . esc_attr( $field_resize_width[ $arf_field_counter ] ) . "'";
}
...
if ( is_array( $field ) ) {
	...
	?>
		<div class="sortable_inner_wrapper edit_field_type_<?php echo esc_attr( $display['type'] ); ?>" id="arfmainfieldid_<?php echo esc_attr( $field['id'] ); ?>" inner_class="<?php echo isset( $field['inner_class'] ) ? esc_attr( $field['inner_class'] ) : 'arf_1col'; ?>" <?php echo esc_attr( $sortable_inner_field_style ); ?>>
			<div id="arf_field_<?php echo esc_attr( $field['id'] ); ?>" class="arfformfield control-group arfmainformfield   <?php echo isset( $newarr['position'] ) ? esc_attr( $newarr['position'] ) . '_container' : 'top_container'; ?> <?php echo ( isset( $newarr['hide_labels'] ) && $newarr['hide_labels'] == 1 ) ? 'none_container' : ''; ?> arf_field_<?php echo esc_attr( $field['id'] ); ?> ">
				...
				<div class="arf_fieldiconbox <?php echo ( $is_edit_option_icon ) ? 'arf_fieldiconbox_with_edit_option' : ''; ?>" data-field_id="<?php echo esc_attr( $field['id'] ); ?>">
					...
					echo $arf_field_require_option_icon = $arflitefieldcontroller->arflite_get_field_control_icons( 'duplicate', '', $field['id'], 0, $field['type'], $arflite_id ); //phpcs:ignore
				...
				<?php } ?>
					...
					switch ( $field['type'] ) {
						...
						case 'captcha':
							...
							<img alt='' id="recaptcha_<?php echo esc_attr( $field['id'] ); ?>" src="<?php echo esc_url( ARFLITEURL ); ?>/images/<?php echo esc_attr( $arflitesettings->re_theme ); ?>-captcha.png" alt="captcha" class="captcha_class" style="max-width:100%;"/>
							...
							<?php
							if ( empty( $arflitesettings->pubkey ) && ( isset( $field['is_recaptcha'] ) && $field['is_recaptcha'] != 'custom-captcha' ) ) {
								?>
												<div class="howto" id="setup_captcha_message" ><?php echo esc_html__( 'Please setup site key and private key in Global Settings otherwise recaptcha will not appear', 'arforms-form-builder' ); ?></div>    <?php } ?>
	...
	if ( ! empty( $confirm_field_options ) ) {
		...
		if ( $confirm_field_options['type'] == 'email' ) {
			...
			if ( $frm_css['arfinputstyle'] != 'material' ) {
				?>
				<div id="arfmainfieldid_<?php echo esc_attr( $field ); ?>" class="sortable_inner_wrapper arf_confirm_field ui-droppable ui-sortable"  inner_class="<?php echo esc_attr( $define_classes ); ?>" <?php echo esc_attr($sortable_inner_field_style); ?>>
		...
		?>
		<div class='sortable_inner_wrapper' inner_class='<?php echo esc_attr( $define_classes ); ?>' <?php echo esc_attr($sortable_inner_field_style); ?>></div>
		<?php
