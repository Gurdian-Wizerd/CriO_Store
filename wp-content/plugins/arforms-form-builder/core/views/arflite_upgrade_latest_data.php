...
if ( version_compare( $arflitenewdbversion, '1.5.1', '<' ) ) {
	...
	$wpdb->query( 'ALTER TABLE `' . $ARFLiteMdlDb->forms . "` ADD `arflite_update_form` TINYINT(1) NOT NULL DEFAULT '0'" ); //phpcs:ignore

	$get_form_ids = $wpdb->get_results( $wpdb->prepare( 'SELECT form_id FROM `' . $ARFLiteMdlDb->fields . '` WHERE ( type = %s OR type = %s OR type = %s ) GROUP BY form_id', 'select', 'checkbox', 'radio' ) ); //phpcs:ignore
