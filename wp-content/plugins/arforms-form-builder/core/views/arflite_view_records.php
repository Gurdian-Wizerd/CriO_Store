<?php
global $arflitemainhelper, $arfliteformhelper, $arfliterecordhelper, $arfliterecordcontroller, $arflitemaincontroller, $arfliteversion, $arflitefieldhelper;
...
if ( isset( $form->id ) && ( $form->id != '-1' || $form->id != '' ) ) {
	...
	$items     = apply_filters( 'arflitepredisplaycolsitems', $items, $form->id );
	...
	$columns_list_res = $wpdb->get_results( $wpdb->prepare( 'SELECT columns_list FROM ' . $ARFLiteMdlDb->forms . ' WHERE id = %d', $form->id ), ARRAY_A ); //phpcs:ignore
...
?>
<input type="hidden" id="ARForms-Lite-page" value="ARForms-Lite-entries" />
<input type="hidden" id="ARForms-Lite-page-item-counter" value="<?php echo esc_attr( count( $items ) ); ?>" />
<input type="hidden" id="ARForms-Lite-page-exclude" value="<?php echo esc_attr( $exclude ); ?>" />
<?php $arfget_form = intval($_GET['form']); ?>
<input type="hidden" id='ARForms-Lite-page-form-id' value="<?php echo esc_attr( $arfget_form ); ?>" />
<input type="hidden" id="ARForms-Lite-page-action-no" value="<?php echo esc_attr( $action_no ); ?>" />
<input type="hidden" id="ARForms-Lite-page-date-format-new" value="<?php echo esc_attr( $date_format_new ); ?>" />
<input type="hidden" id="ARForms-Lite-page-date-format-new1" value="<?php echo esc_attr( $date_format_new1 ); ?>" />
<input type="hidden" id="ARForms-Lite-page-start-date-new" value="<?php echo esc_attr( $start_date_new ); ?>" />
<input type="hidden" id="ARForms-Lite-page-end-date-new" value="<?php echo esc_attr( $end_date_new ); ?>" />
...
<div class="wrap frm_entries_page">
	...
	<div id="poststuff" class="metabox-holder">
		...
		<div id="post-body">
			<div class="inside" style="background-color:#ffffff;">
				<div class="formsettings1" style="background-color:#ffffff;">
					...
					<div class="frm_settings_form">
						...
						<input type="hidden" name="arfformentriesurl" id="arfformentriesurl" value="<?php echo esc_url( admin_url( 'admin.php' ) . '?page=ARForms-Lite-entries' ); ?>" />
						...
						<div id="form_entries">
							<div class="arf_form_entry_select">
								<table class="arf_form_entry_select_sub">
									...
									<tr>
										<td><div class="sltstandard" style="float:none; width: 400px !important;<?php echo ( is_rtl() ) ? 'margin-left:60px;' : 'margin-right:60px;'; ?>margin-top:-16px;"><?php $arfliteformhelper->arflite_forms_dropdown( 'arfredirecttolist', intval( $_GET['form'] ), __( 'Select Form', 'arforms-form-builder' ), false, '' ); ?></div></td>
										<td>
											<?php
											if ( is_rtl() ) {
											...
											<div style="position:relative; <?php echo esc_attr( $sel_frm_date_wrap ); ?>">
												<div style="<?php echo esc_attr( $sel_frm_sel_date ); ?>"><div class="arfentrytitle" style='margin-left:0;'><?php echo __( 'From', 'arforms-form-builder' ); ?></div><input type="text" class="txtmodal1" value="<?php echo ( !empty( sanitize_text_field($_GET['start_date']) ) ) ?  esc_attr( $_GET['start_date'] ) : ''; ?>" id="datepicker_from" name="datepicker_from" style="width:120px;height:35px;vertical-align:middle; " /></div> <div class="arfentrytitle"><?php echo __( 'To', 'arforms-form-builder' ); ?></div>&nbsp;&nbsp;<div style="<?php echo esc_attr( $sel_frm_sel_date ); ?>"><input type="text" class="txtmodal1" value="<?php echo ( !empty( $_GET['end_date'] ) ) ? sanitize_text_field( $_GET['end_date'] ) : ''; ?>" id="datepicker_to" name="datepicker_to" style="vertical-align:middle; width:120px;height:35px;"/></div> <?php //phpcs:ignore ?>
												<div style=" <?php echo esc_attr( $sel_frm_button ); ?>">
													<div class="arf_form_entry_left">&nbsp;</div>
													<div style="float:left;text-align:left;"><button type="button" class="rounded_button arf_btn_dark_blue" onclick="arflite_change_frm_entries();" style="width: 35px !important;height: 35px;"><?php echo esc_html__( 'Go', 'arforms-form-builder' ); ?></button></div>
												</div>
			...
			?>
			<div class="documentation_link"  align="<?php echo esc_attr( $doc_link_align ); ?>"><a href="https://www.arformsplugin.com/documentation/1-getting-started-with-arforms/"  target="_blank" class="arlinks doc-link-a"><?php echo esc_html__( 'Documentation', 'arforms-form-builder' ); ?></a></div>
			...
			<div class="arf_entry_popup_container_wrapper">
				<div class='arf_popup_container arf_view_entry_modal arf_popup_container_view_entry_modal'>
					...
					<div class="arf_popup_footer arf_view_entry_modal_footer">
						  <div class="arflite_navigation_button">
							<button class="rounded_button arf_btn_dark_blue" id="arflite_prev_entry_button" name="arflite_prev_entry_button" style="<?php echo ( is_rtl() ) ? 'margin-left:7px;' : 'margin-right:7px;'; ?>"><?php echo esc_html__( 'Previous Entry', 'arforms-form-builder' ); ?></button>

							<button class="rounded_button arf_btn_dark_blue" id="arflite_next_entry_button" name="arflite_next_entry_button"><?php echo esc_html__( 'Next Entry', 'arforms-form-builder' ); ?></button>
					</div>
