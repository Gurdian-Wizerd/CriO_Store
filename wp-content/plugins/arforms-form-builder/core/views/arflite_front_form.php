...
if ( ! function_exists( 'arflite_get_form_builder_string' ) ) {
	...
	function arflite_get_form_builder_string( $id, $key = '', $arflite_preview = 0, $is_widget_or_modal = 0, $arflite_errors = array(), $arflite_data_uniq_id = '', $desc = '', $type = '', $modal_height = '', $modal_width = '', $position = '', $btn_angle = '', $bgcolor = '', $txtcolor = '', $open_inactivity = '', $open_scroll = '', $open_delay = '', $overlay = '', $is_close_link = '', $modal_bgcolor = '', $is_fullscrn = '', $inactive_min = '', $model_effect = '', $navigation = false, $arf_preset_data = '', $hide_popup_for_loggedin_user = 'no' ) {
		...
		$arflite_front_form_useragent = isset( $_SERVER['HTTP_USER_AGENT'] ) ? $_SERVER['HTTP_USER_AGENT'] : '';
		...
		if ( ( is_object( $form ) && ( isset( $form->is_template ) && ( ! isset( $form->status ) || $form->status == 'draft' ) ) ) && ! ( $arflite_preview ) ) {
			...
			return do_shortcode( $arflitesettings->login_msg );
		}
		...
		if ( is_array( $form->form_css ) ) {
			...
			if ( $form->form_css['arfsubmitbuttontext'] != '' ) {
				...
				$submit = $arflitesettings->submit_value;
			}
		} else {
			$submit = $arflitesettings->submit_value;
		}
		...
		if ( false == $fields ) {
			...
			$saved_message = isset( $form_options['success_msg'] ) ? '<div id="arf_message_success"><div class="msg-detail"><div class="msg-description-success">' . $form_options['success_msg'] . '</div></div></div>' : $arflitesettings->success_msg;

			$saved_popup_message = isset( $form_options['success_msg'] ) ? '<div id="arf_message_success_popup" style="display: none;"><div class="msg-detail"><div class="msg-description-success">' . $form_options['success_msg'] . '</div></div></div>' : '<div id="arf_message_success_popup" style="display: none;" ><div class="msg-detail"><div class="msg-description-success">' . $arflitesettings->success_msg . '</div></div></div>';

		if ( $params['action'] == 'create' && $params['posted_form_id'] == $form->id && isset( $_POST ) ) { //phpcs:ignore

			if ( isset( $_REQUEST['arfsubmiterrormsg'] ) ) {

				$arferror_message = ( $_REQUEST['arfsubmiterrormsg'] != '' ) ? sanitize_text_field( $_REQUEST['arfsubmiterrormsg'] ) : $arflitesettings->failed_msg;
			...
			if ( ! empty( $arflite_errors ) ) {
				...
				if ( apply_filters( 'arflitecontinuetocreate', true, $form->id ) ) {
					...
					if ( $arflitesettings->form_submit_type != 1 && $conf_method == 'redirect' && $saved_message == false ) {
						$conf_method         = 'message';
						$saved_message       = '<div  id="arf_message_success"><div class="msg-detail"><div class="msg-description-success">' . $arflitesettings->success_msg . '</div></div></div>';
						$saved_popup_message = '<div id="arf_message_success_popup" style="display: none;"><div class="msg-detail"><div class="msg-description-success">' . $arflitesettings->success_msg . '</div>';
					}
					...
					if ( ! $created || ! is_numeric( $created ) || $conf_method == 'message' ) {
						...
						$failed_msg = '<div class="frm_error_style" id="arf_message_error"><div class="msg-detail"><div class="arf_res_front_msg_desc">' . $arflitesettings->failed_msg . '</div></div></div>';
						...
						if ( $conf_method == 'page' && is_numeric( $form_options['success_page_id'] ) ) {
							...
							if ( $form_options['success_page_id'] != $post->ID ) {
								...
								foreach ( $matches[0] as $key => $val ) {
									...
									$temp_form_opts = $wpdb->get_row( $wpdb->prepare( 'SELECT `form_css` FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $arf_form_id_extracted ) ); //phpcs:ignore
		...
		if ( 1 != $arflitesettings->hidden_captcha ) {
		...
		if ( isset( $arflite_preview ) && $arflite_preview ) {
			...
			$action_html = ( $arflitesettings->use_html ) ? '' : 'action=""';
		...
		if ( $is_recaptcha == 1 ) {
			$arf_form .= "<input type='hidden' id='arf_settings_recaptcha_v2_public_key' value='" . esc_attr( $arflitesettings->pubkey ) . "' />";
			$arf_form .= "<input type='hidden' id='arf_settings_recaptcha_v2_public_theme' value='" . esc_attr( $arflitesettings->re_theme ) . "' />";
			$arf_form .= "<input type='hidden' id='arf_settings_recaptcha_v2_public_lang' value='" . esc_attr( $arflitesettings->re_lang ) . "' />";
		}
		...
		if ( ! empty( $_REQUEST['action'] ) && 'elementor' == $_REQUEST['action'] ) {
			...
			do_action( 'arflite_load_assets_for_elementor', intval( $form->id ), sanitize_text_field( $_REQUEST['action'] ) );
