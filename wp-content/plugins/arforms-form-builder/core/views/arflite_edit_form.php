...
global $arflitefieldhelper, $arfliteformhelper, $ARFLiteMdlDb, $arflite_fields_with_external_js, $arflite_bootstraped_fields_array,$arfliteformcontroller;
			...
			<div id="new_fields" data-flag="1" class="newfield_div">
				...
				if ( isset( $values['fields'] ) && ! empty( $values['fields'] ) ) {
					...
					if ( $arfaction == 'duplicate' ) {
						$arf_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE `form_id` = %d', $define_template ), ARRAY_A ); //phpcs:ignore
					} elseif ( $arfaction == 'edit' ) { 
						$arf_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE `form_id` = %d', $arflite_id ), ARRAY_A ); //phpcs:ignore
					}
