...
if ( ! function_exists( 'arflite_display_form_preview' ) ) {
	...
	function arflite_display_form_preview( $form_id, $form_key, $posted_data = array() ) {
		...
		global $arfliteform, $user_ID, $arflitesettings, $post, $wpdb, $arflitemainhelper, $arfliterecordcontroller, $arfliteformcontroller, $arflitefieldhelper, $arfliterecordhelper, $arflite_forms_loaded, $arflite_form_all_footer_js, $arflitecreatedentry, $ARFLiteMdlDb, $arfliteformhelper;

		$arf_preview_useragent = isset( $_SERVER['HTTP_USER_AGENT'] ) ? $_SERVER['HTTP_USER_AGENT'] : '';
		...
		if ( $form->form_css['arfsuccessmsgposition'] == 'top' ) {
			$saved_message = isset( $form->options['success_msg'] ) ? '<div id="arf_message_success"><div class="msg-detail"><div class="msg-description-success">' . $form->options['success_msg'] . '</div></div></div>' : $arflitesettings->success_msg;
