...
class arflitesettingcontroller {
	...
	function __construct() {
		...
		add_action( 'admin_init', array( $this, 'arflite_admin_init' ) );

		add_action( 'admin_menu', array( $this, 'arflitemenu' ), 26 );
		...
		add_action( 'wp_ajax_arflite_send_test_mail', array( $this, 'arflite_send_test_mail' ) );
	...
	function arflite_send_test_mail() {
	...
	function arfliteroute() {
		...
		if ( isset( $_REQUEST['page'] ) && sanitize_text_field( $_REQUEST['page'] ) == 'ARForms-Lite-import-export' ) {
			return $arflitesettingcontroller->arflite_import_export_form();
		} elseif ( isset( $_REQUEST['page'] ) && sanitize_text_field( $_REQUEST['page'] ) == 'ARForms-Lite-addons' ) {
	...
	function arflitedisplay_form() {
		...
		global $arflitesettings, $arfliteajaxurl, $wpdb, $arfliteform, $arflitemainhelper, $ARFLiteMdlDb;
		...
		if ( get_option( 'arflite_current_tab' ) == '' ) {

			update_option( 'arflite_current_tab', sanitize_text_field( 'general_settings' ) );
		}
	...
	function arflite_import_export_form() {
		require ARFLITE_VIEWS_PATH . '/arflite_import_export_form.php';
	}

	function arfliteprocess_form( $cur_tab = '' ) {

		global $arflitesettings, $arfliteajaxurl, $wpdb, $ARFLiteMdlDb;

		$arflite_errors = array();

		if ( $cur_tab == 'general_settings' ) {

			$arflitesettings->arfliteupdate( $_POST, $cur_tab ); //phpcs:ignore

		}

		if ( $cur_tab != '' ) {

			update_option( 'arflite_current_tab', sanitize_text_field( $cur_tab ) );
		}

		if ( empty( $arflite_errors ) ) {

			$arflitesettings->arflitestore( $cur_tab );
	...
	function arflite_admin_init() {

		global $arflitesettings;

		add_action( 'admin_head-' . sanitize_title( $arflitesettings->menu ) . '_page_ARForms-Lite-settings', array( $this, 'arflitehead' ) );
	}
