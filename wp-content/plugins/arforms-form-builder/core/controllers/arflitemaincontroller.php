...
class arflitemaincontroller {
	...
	function __construct() {
		...
		add_action( 'wp_ajax_arflite_check_export_form_data_entry', array( $this, 'arflite_check_export_form_data_entry_func' ) );

	}

	function arflite_check_export_form_data_entry_func() {
		...
		$arflite_all_form_id     = !empty( $_POST['form_id'] ) ? intval( $_POST['form_id'] ) : '';
		$arflite_all_form_id_arr = explode( ',', $arflite_all_form_id );

		foreach ( $arflite_all_form_id_arr as $arflite_all_form_id_arr_key => $arflite_all_form_id_arr_val ) {
			...
			$date_from = date( 'Y-m-d', strtotime( $arfdatefrom ) );
			$date_to   = date( 'Y-m-d', strtotime( $arfdateto ) );

			$arflite_form_entry_ids = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM `' . $ARFLiteMdlDb->entries . '` WHERE form_id = %d AND created_date BETWEEN %s AND %s ', $arflite_all_form_id_arr_val, $date_from, $date_to ) );//phpcs:ignore

			if ( count( $arflite_form_entry_ids ) > 0 ) {
				echo 1;
			} else {
				echo 0;
			}
		...
		if ( is_admin() ) {
			...
			if ( isset( $_GET['page'] ) && 'ARForms-Lite' == sanitize_text_field( $_GET['page'] ) ) {
				...
				if ( ! isset( $_GET['arflite_page_nonce'] ) ) {
					...
					$total = $wpdb->get_var( $wpdb->prepare( 'SELECT count(*) as total FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $form_id ) ); //phpcs:ignore
	...
	function arflite_enqueue_gutenberg_assets() {
		...
		if ( in_array( $page, array( 'post.php', 'page.php', 'page-new.php', 'post-new.php' ) ) || ( isset( $_GET ) && isset( $_GET['page'] ) && sanitize_text_field( $_GET['page'] ) == 'ARForms-Lite-entry-templates' ) ) {
			...
			$arforms_forms_lite_data = $wpdb->get_results( 'SELECT * FROM `' . $arformslite_forms . "` WHERE is_template=0 AND (status is NULL OR status = '' OR status = 'published') ORDER BY id DESC" );//phpcs:ignore
	...
	function arflite_update_auto_increment_after_install() {
		global $wpdb, $ARFLiteMdlDb;
		$result_1 = $wpdb->get_results( "SHOW TABLE STATUS LIKE '" . $ARFLiteMdlDb->forms . "'" ); //phpcs:ignore
		if ( $result_1[0]->Auto_increment < 100 ) {
			$wpdb->query( "ALTER TABLE {$ARFLiteMdlDb->forms} AUTO_INCREMENT = 100" ); //phpcs:ignore
		}
	}
		...
		if ( ! is_admin() ) {
			...
			if ( is_array( $posts ) ) {
				foreach ( $posts as $post_data ) {
					if ( preg_match_all( '/' . $pattern . '/s', $post_data->post_content, $matches ) && array_key_exists( 2, $matches ) && in_array( 'ARFormslite', $matches[2] ) ) {
						break;
					}
				}
			...
			if ( is_array( $newvalarr ) && count( $newvalarr ) > 0 ) {
				...
				foreach ( $newvalarr as $newkey => $newval ) {
					...
					if ( ! $res ) {
						$res = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->forms . ' WHERE id = %d', $newval ), 'ARRAY_A' ); //phpcs:ignore
	...
	function arflite_get_form_shortcode( $atts ) {
		...
		if ( false == $res ) {
			$res = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->forms . ' WHERE id = %d', $id ), 'ARRAY_A' );//phpcs:ignore
	...
	function arflite_load_assets_for_elementor_callback( $form_id = '', $action = '' ) {
		...
		global $arflite_jscss_version, $arfliterecordcontroller, $wpdb, $ARFLiteMdlDb;

		if ( empty( $form_id ) ) {
			return;
		}

		$temp_form_opts = $wpdb->get_row( $wpdb->prepare( 'SELECT `form_css` FROM `' . $ARFLiteMdlDb->forms . '` WHERE id = %d', $form_id ) );//phpcs:ignore
	...
	function arflite_set_js( $hook ) {
		...
		$page = !empty( $_SERVER['PHP_SELF']) ? basename( $_SERVER['PHP_SELF'] ) : '';
	...
	function arflite_set_css() {
		...
		if ( ( isset( $_REQUEST['page'] ) && $_REQUEST['page'] != '' ) && ( sanitize_text_field( $_REQUEST['page'] ) == 'ARForms-Lite-entries' || sanitize_text_field( $_REQUEST['page'] ) == 'ARForms-Lite-import-export' ) ) {
	...
	function arflite_upgrade_data() {
		...
		if ( version_compare( $arflitenewdbversion, '1.5.6', '<' ) ) {
	...
	function arflite_export_form_data() {
		...
		if ( isset( $_POST['s_action'] ) && ! in_array( sanitize_text_field( $_POST['s_action'] ), array( 'arflite_opt_export_form', 'arflite_opt_export_both' ) ) ) { //phpcs:ignore
			return false;
		}
		...
		if ( isset( $_REQUEST['export_button'] ) ) {
			if ( ! empty( $form_id_req ) ) {
				...
				$res = $wpdb->get_results( 'SELECT * FROM ' . $ARFLiteMdlDb->forms . ' WHERE id in (' . $form_ids . ')' ); //phpcs:ignore
				...
				foreach ( $res as $key => $result_array ) {
					...
					foreach ( $result_array as $key => $value ) {
						...
						$res_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->fields . ' WHERE form_id = %d', $result_array->id ) ); //phpcs:ignore
					...
					if ( !empty( $_REQUEST['arflite_opt_export'] ) && sanitize_text_field( $_REQUEST['arflite_opt_export'] ) == 'arflite_opt_export_both' ) {

						global $wpdb, $arfliteform, $arflitefield, $arflite_db_record, $arflite_style_settings, $arflitemainhelper, $arflitefieldhelper, $arfliterecordhelper,$ARFLiteMdlDb;
						...
						if ( ! empty( $_REQUEST['datepicker_from2'] ) || ! empty( $_REQUEST['datepicker_to2'] ) ) {
							...
							$form_entry_ids = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM `' . $ARFLiteMdlDb->entries . '` WHERE form_id = %d AND created_date BETWEEN %s AND %s ', $form_id, $date_from, $date_to ) ); //phpcs:ignore
						} else {
							$form_entry_ids = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM `' . $ARFLiteMdlDb->entries . '` WHERE form_id = %d', $form_id ) ); //phpcs:ignore
						}
						...
						foreach ( $entries as $entry ) {
							...
							$get_form_submit_type = $wpdb->get_results( $wpdb->prepare( 'SELECT entry_value FROM ' . $ARFLiteMdlDb->entry_metas . ' WHERE entry_id = %d and field_id = %d', $entry->id, 0 ), 'ARRAY_A' ); //phpcs:ignore

							$form_submit_type = $get_form_submit_type[0]['entry_value'];

							$res_data = $wpdb->get_results( $wpdb->prepare( 'SELECT country, browser_info FROM ' . $ARFLiteMdlDb->entries . ' WHERE id = %d', $entry->id ), 'ARRAY_A' ); //phpcs:ignore
                                ...
                                <div class="arf_whatsnew_popup_inner_content">
                                    ...
                                    You can always refer our online documentation for all the features <a href="https://www.arformsplugin.com/documentation/1-getting-started-with-arforms/" target="_blank">here</a><br>
                                        <ul style="list-style-type: disc;">
											<li>Fixed deprecation and other notices with PHP 8.2</li>
											<li>Added compatibility for WordPress 6.2</li>
