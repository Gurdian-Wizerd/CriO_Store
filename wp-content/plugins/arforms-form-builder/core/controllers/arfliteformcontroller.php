...
class arfliteformcontroller {
	...
	function arflite_add_action_links( $links ) {
		...
		$mylinks[] = '<a href="https://www.arformsplugin.com" target="_blank" rel="noopener noreferrer" style="font-weight:bold;">' . __( 'Upgrade To Premium', 'arforms-form-builder' ) . '</a>';

		$mylinks[] = '<a href="' . esc_url_raw( admin_url( 'admin.php?page=ARForms-Lite-addons' ) ) . '">Addons</a>';
	...
	function arflite_class_to_hide_form( $id, $hide_form = false ) {
		...
		$table = $ARFLiteMdlDb->forms;

		$form_data = wp_cache_get( 'arflite_form_options_' . $id );
		if ( false == $form_data ) {
			$form_data = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM `' . $table . '` WHERE `id` = %d', $id ) ); //phpcs:ignore
	...
	function arflite_process_bulk_form_actions( $arflite_errors ) {
		...
		if ( isset( $_GET['arflite_page_nonce'] ) && '' != $_GET['arflite_page_nonce'] && wp_verify_nonce( sanitize_text_field( $_GET['arflite_page_nonce'] ), 'arflite_page_nonce' ) ) {
			return;
		}
		...
		if ( empty( $ids ) ) {
			...
			if ( ! current_user_can( 'arfdeleteforms' ) ) {
				...
				$arflite_errors[] = $arflitesettings->admin_permission;
			} else {
	...
	function arflite_list_form() {
		...
		$$arflite_errors = $return_array['error'];
	...
	function ARFormslite_shortcode_atts( $atts ) {
		global $arflitereadonly, $arfliteformcontroller, $arfliteeditingentry, $arfliteshowfields, $ARFLiteMdlDb, $wpdb, $arflite_fid;
		$arflite_fid         = $atts['id'];
		...
		if ( $atts['entry_id'] == 'last' ) {
			...
			if ( $user_ID ) {
				...
				$arfliteeditingentry = $ARFLiteMdlDb->arfliteget_var( $ARFLiteMdlDb->entries, $where_meta, 'id', 'created_date DESC' );
			}
	...
	function arflite_filter_content( $content, $form, $entry = false ) {
		...
		if ( $entry && is_numeric( $entry ) ) {
			...
			if ( isset( $_POST['arflite_entry_nonce'] ) && '' != $_POST['arflite_entry_nonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['arflite_entry_nonce'] ), 'arflite_entry_nonce' ) ) {
				return $content;
			}
	...
	function arflitepreview( $form_key = '' ) {
		...
		global $arfliteform, $arflitesettings, $arflitemainhelper, $arfliterecordcontroller, $arflitemaincontroller;
	...
	function arflitedestroy() {
		...
		if ( ! current_user_can( 'arfdeleteforms' ) ) {
			...
			wp_die( esc_attr( $arflitesettings->admin_permission ) );
		}
	...
	function arflite_display_forms_list( $params = false, $message = '', $page_params_ov = false, $current_page_ov = false, $arflite_errors = array() ) {
		...
		global $wpdb, $ARFLiteMdlDb, $arflitemainhelper, $arfliteform, $arflite_db_record, $arflitepagesize;
		...
		if ( $message == '' ) {
			...
			$page_params = '&action=0&&arfaction=0&page=ARForms-Lite';
		...
		$record_count = $arflitemainhelper->arflitegetRecordCount( $form_vars['where_clause'], $ARFLiteMdlDb->forms );

		$page_count = $arflitemainhelper->arflitegetPageCount( $arflitepagesize, $record_count, $ARFLiteMdlDb->forms );

		$forms = $arflitemainhelper->arflitegetPage( $current_page, $arflitepagesize, $form_vars['where_clause'], $form_vars['order_by'], $ARFLiteMdlDb->forms );
	...
	function arflite_change_show_hide_column() {
		...
		if ( isset( $_POST['arflite_wp_nonce'] ) && '' != $_POST['arflite_wp_nonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['arflite_wp_nonce'] ), 'arflite_wp_nonce' ) ) {
			echo '';
			die;
		}
		...
		update_option( 'arfliteformcolumnlist', $ser_arr );
	...
	function arfliteupdateformbulkoption() {
		...
		if ( isset( $_POST['_arflite_wpnonce'] ) && '' != $_POST['_arflite_wpnonce'] && ! wp_verify_nonce( sanitize_text_field( $_POST['_arflite_wpnonce'] ), 'arflite_wp_nonce' ) ) {
			echo json_encode(
				array(
					'error'       => true,
		...
		if ( $action1 == 'bulk_delete' || $action2 == 'bulk_delete' ) {
			...
			$query  = 'DELETE FROM ' . $ARFLiteMdlDb->forms . ' ' . $where;
			...
			if ( $wpdb->last_error != '' ) {
				...
				$totalRecord = $wpdb->get_results( $wpdb->prepare( 'SELECT COUNT(*) as total_forms FROM ' . $ARFLiteMdlDb->forms . ' ' . $where . ' ', 0 ) ); //phpcs:ignore
	...
	function arflite_load_form_grid_data() {
		global $wpdb, $arflite_db_record, $ARFLiteMdlDb;
		...
		$query = $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->forms . " WHERE is_template = %d AND (status is NULL OR status = '' OR status = 'published') ORDER BY id DESC", 0 ); //phpcs:ignore
			...
			foreach ( $grid_columns as $key => $tmp_data ) {
				switch ( $key ) {
					...
					case 'entries':
						...
						$data   .= "<td class='entry_column'>" . ( ( current_user_can( 'arfviewentries' ) ) ? "<a href='" . esc_url( admin_url( 'admin.php' ) . '?page=ARForms-Lite-entries&form=' . $form_data->id ) . "'>" . $entries . '</a>' : $entries ) . '</td>';
						$ni++;
						break;
					case 'shortcode':
                        ...
                        <div class='arf_shortcode_div'>
                            ...
                            <input type='text' class='shortcode_textfield' readonly='readonly' onclick='this.select();' onfocus='this.select();' value='[ARFormslite id=" . esc_attr( $form_data->id ) . "]' />
                        </div></td>";
						$ni++;
						break;
					case 'created_date':
	...
	function arfliteformsavealloptions() {
		global $arfliteform, $wpdb, $ARFLiteMdlDb, $arflitemainhelper, $arflitesettingcontroller, $arflitesettings, $arflitefield, $arfliteformhelper, $arflitefieldhelper, $arflitemaincontroller;
		...
		if ( isset( $values['form_key'] ) ) {
			$values['form_key'] = $arflitemainhelper->arflite_get_unique_key( $values['form_key'], $ARFLiteMdlDb->forms, 'form_key', $id );
		}
		...
		$sel_fields = $wpdb->prepare( 'SELECT id FROM ' . $ARFLiteMdlDb->fields . ' where form_id = %d', $id ); //phpcs:ignore
		...
		if ( ! $is_preview ) {
			if ( ! empty( $form_css ) ) {
				...
				if ( $str['arfaction'] == 'new' || $str['arfaction'] == 'duplicate' ) {
					$db_data['form_key']     = $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->forms, 'form_key' );
					$db_data['created_date'] = date( 'Y-m-d H:i:s' );
					$query_results           = $wpdb->insert( $ARFLiteMdlDb->forms, $db_data );
					...
					if ( ! empty( $db_data ) ) {
						$query_results = $wpdb->update( $ARFLiteMdlDb->forms, $db_data, array( 'id' => $id ) );
						if ( $query_results ) {
				...
				if ( in_array( $field_id_new, $old_field_array ) ) {
					...
					if ( $field_otions_new['type'] == 'email' ) {
						if ( $field_otions_new['confirm_email'] == '1' ) {
							$email_field_key = $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->fields, 'field_key' );
					...
					if ( ! $is_preview ) {
						$update = $wpdb->update( $ARFLiteMdlDb->fields, $field_data_to_save, array( 'id' => $field_id_new ) );
					} else {
						if ( ! isset( $arf_preview_form_data['fields'] ) ) {
							$arf_preview_form_data['fields'] = array();
						}
						$field_data_to_save['id']          = $field_id_new;
						$field_data_to_save['field_key']   = $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->fields, 'field_key' );
					...
					$field_key      = $arflitemainhelper->arflite_get_unique_key( '', $ARFLiteMdlDb->fields, 'field_key' );
					...
					if ( ! $is_preview ) {
						$wpdb->insert( $ARFLiteMdlDb->fields, $args, $format );
					} else {
						if ( ! isset( $arf_preview_form_data['fields'] ) ) {
							$arf_preview_form_data['fields'] = array();
						}
		...
		if ( isset( $changed_field_value ) && ! empty( $changed_field_value ) ) {
			$selectDeletedFields = $wpdb->get_results( $wpdb->prepare( 'SELECT id FROM `' . $ARFLiteMdlDb->fields . "` WHERE id NOT IN( '" . implode( '\',\'', $changed_field_value ) . "') AND form_id = %d", $id ) ); //phpcs:ignore
			$del_fields          = $wpdb->query( $wpdb->prepare( 'delete from ' . $ARFLiteMdlDb->fields . ' where form_id = %d and id NOT IN (' . implode( ',', $changed_field_value ) . ')', $id ) ); //phpcs:ignore
		} elseif ( empty( $changed_field_value ) ) {
			$del_fields = $wpdb->query( $wpdb->prepare( 'delete from ' . $ARFLiteMdlDb->fields . ' where form_id = %d', $id ) ); //phpcs:ignore
		}
		if ( ! $is_preview ) {
			$query_results = $wpdb->query( 'update ' . $ARFLiteMdlDb->forms . " set options = '" . addslashes( maybe_serialize( $options ) ) . "', temp_fields='" . maybe_serialize( $arf_temp_fields ) . "' where id = '" . $id . "'" ); //phpcs:ignore
		} else {
			$arf_preview_form_data['options']     = $options;
		...
		$sel_fields = $wpdb->prepare( 'select * from ' . $ARFLiteMdlDb->fields . ' where form_id = %d', $str['id'] ); //phpcs:ignore
		...
		$all_fields     = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE form_id = %d', $id ) ); //phpcs:ignore
		...
		if ( isset( $values['fields'] ) && ! empty( $values['fields'] ) ) {
			...
			$arf_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE `form_id` = %d', $id ), ARRAY_A ); //phpcs:ignore
	...
	function arflitedeciamlseparator( $value = 0 ) {
		global $arflitesettings, $arflite_decimal_separator;
		$value                     = number_format( (float) $value, 2 );
		$arflite_decimal_separator = $arflitesettings->decimal_separator;
	...
	function arflite_get_form_hidden_field( $form, $fields, $values, $arflite_preview, $is_widget_or_modal, $arflite_data_uniq_id, $form_action, $loaded_field, $type, $is_close_link, $arf_current_token ) {
		...
		$hidden_fields = '';
		global $arfliterecordcontroller, $arflitesettings, $arflitefieldhelper, $arflite_form_all_footer_js,$arflite_decimal_separator;

		$arflite_http_server_agent = !empty( $_SERVER['HTTP_USER_AGENT'] ) ? $_SERVER['HTTP_USER_AGENT'] : '';
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arf_success_message_show_time_' . esc_attr( $form->id ) . '" data-id="arf_success_message_show_time_' . esc_attr( $form->id ) . '" value="' . esc_attr( $arflitesettings->arf_success_message_show_time ) . '" />';
		...
		if ( '1' == $arflitesettings->arfmainformloadjscss ) {
			$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="is_load_js_and_css_in_all_pages" data-id="is_load_js_and_css_in_all_pages" value="' . esc_attr( $arflitesettings->arfmainformloadjscss ) . '" />';
		}
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="form_submit_type" data-id="form_submit_type" value="' . esc_attr( $arflitesettings->form_submit_type ) . '" />';

		$_SERVER['HTTP_REFERER'] = !empty( $_SERVER['HTTP_REFERER'] ) ? esc_url_raw($_SERVER['HTTP_REFERER']) : '';
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arf_http_referrer_url" data-id="arf_http_referrer_url" value="' . esc_attr($_SERVER['HTTP_REFERER']) . '" />';
		...
		$hidden_fields .= '<input type="hidden" data-jqvalidate="false" name="arflite_skip_captcha" data-id="arflite_skip_captcha" value="' . esc_attr( $arflitesettings->hidden_captcha ) . '" />';
	...
	function arflite_get_all_field_html( $form, $values, $arflite_data_uniq_id, $fields, $arflite_preview, $arflite_errors, $inputStyle, $arf_arr_preset_data = array() ) {
		...
		global $arflitefieldhelper, $arfliteformcontroller, $arflitefieldcontroller, $arflitemainhelper, $arfliterecordcontroller, $arflitesettings, $arflite_form_all_footer_js, $arflitemaincontroller, $wpdb, $ARFLiteMdlDb, $arfliteversion,$arflite_style_settings,$arflite_form_all_footer_css;
		$return_string       = '';
		$arf_classes_blank   = '';
		$confirm_email_style = '';
		...
		if ( false == $OFData ) {
			$OFData = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->fields . ' WHERE form_id = %d ORDER BY id', $form->id ) ); //phpcs:ignore
				...
				switch ( $field['type'] ) {
					...
					case 'number':
						...
						if ( apply_filters( 'arflite_check_for_draw_outside', false, $field ) ) {
							...
							$return_string .= $prefix;
							$num_field_type = 'text';
							$arflitehttpserver = !empty($_SERVER['HTTP_USER_AGENT']) ? $_SERVER['HTTP_USER_AGENT'] : '';
	...
	function arflite_delete_form_function() {
		global $wpdb, $ARFLiteMdlDb,$arfliteform,$arflitemaincontroller;
	...
	function arflite_change_input_style() {
		...
		$arf_fields = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->fields . '` WHERE `form_id` = %d', $form_id ), ARRAY_A ); //phpcs:ignore
		asort( $field_order );
		$arf_sorted_fields = array();
		foreach ( $field_order as $field_id => $order ) {
			if ( is_int( $field_id ) ) {
		...
		if ( is_array( $styling_opts ) && ! empty( $styling_opts ) && count( $styling_opts ) > 0 ) {
			...
			$arf_bg_position_input_y = !empty(intval($_REQUEST['arf_bg_position_input_y'])) ? intval($_REQUEST['arf_bg_position_input_y']) : '';
	...
	function arflite_load_form_css( $form_id, $inputStyle ) {
		global $arflitesettings, $arflite_jscss_version, $arfliteversion;
		...
		if ( ! wp_style_is( $stylesheet_handler, 'enqueued' ) ) {
			if ( $arflitesettings->arfmainformloadjscss != 1 ) {
				wp_enqueue_style( $stylesheet_handler, $fid, array(), $arflite_jscss_version );
			} else {
				$new_key = '';
				global $ARFLiteMdlDb,$arflitemainhelper;
				$unique_key = $arflitemainhelper->arflite_get_unique_key( $new_key, $ARFLiteMdlDb->forms, 'form_key' );
			}
		} else {
	...
	function arflite_rewrite_form_css( $form_id, $frm_css ) {
		global $wpdb, $ARFLiteMdlDb, $arfliteform;
		...
		$temp_fres = $wpdb->get_results( $wpdb->prepare( 'SELECT * FROM ' . $ARFLiteMdlDb->fields . ' WHERE form_id = %d ORDER BY id', $form->id ) ); //phpcs:ignore
