...
class arf_element_shortcode extends Widget_Base {
	...
	protected function _register_controls() {
		...
		$where                                   = apply_filters( 'arfliteformsdropdowm', "is_template=0 AND (status is NULL OR status = '' OR status = 'published')", 'arf_select' );
	...
	protected function render() {
		...
		if ( isset( $settings['arf_select'] ) && $settings['arf_select'] == 'Please select a valid form' ) {
			...
			echo do_shortcode( '[ARFormslite ' . $settings['arf_select'] . ' is_elementor="yes"]' );
		}
		echo '</div>';
