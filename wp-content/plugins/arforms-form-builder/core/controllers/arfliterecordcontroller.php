...
class arfliterecordcontroller {
	...
	function arfliteprocess_entry( $arflite_errors = '' ) {
		...
		if ( empty( $arflite_errors ) && @count( $arf_errors ) == 0 ) {
			...
			if ( isset( $_POST['form_submit_type'] ) && intval( $_POST['form_submit_type'] ) == 1 ) { //phpcs:ignore
				...
				do_shortcode( '[ARFormslite id=' . intval( $_POST['form_id'] ) . " arfsubmiterrormsg='" . $arferrormsg . "' ]" ); //phpcs:ignore
			}
		}

		if ( isset( $_POST['using_ajax'] ) && sanitize_text_field( $_POST['using_ajax'] ) == 'yes' ) { //phpcs:ignore
			echo do_shortcode( '[ARFormslite id=' . intval( $_POST['form_id'] ) . ']' ); //phpcs:ignore
		}
	}
	...
	function arflite_admin_js() {
		...
		if ( isset( $_GET ) && isset( $_GET['page'] ) && ( 'ARForms-popups' == sanitize_text_field( $_GET['page'] ) || sanitize_text_field( $_GET['page'] ) == 'ARForms-Lite-entries' || sanitize_text_field( $_GET['page'] ) == 'ARForms-entry-templates' || sanitize_text_field( $_GET['page'] ) == 'ARForms-Lite-import' || ( isset( $_REQUEST['page'] ) && sanitize_text_field( $_REQUEST['page'] == 'ARForms-Lite') ) && ( ( isset( $_REQUEST['arfaction'] ) && sanitize_text_field( $_REQUEST['arfaction'] ) == 'edit' ) || ( isset( $_REQUEST['arfaction'] ) && sanitize_text_field( $_REQUEST['arfaction'] ) == 'new' ) || ( isset( $_REQUEST['arfaction'] ) && sanitize_text_field( $_REQUEST['arfaction'] ) ) == 'duplicate' || ( isset( $_REQUEST['arfaction'] ) && sanitize_text_field( $_REQUEST['arfaction'] ) == 'update' ) ) ) ) {
	...
	function arflite_footer_js( $arflite_preview = false, $is_print = false, $force_load = false ) {

		global $wp_version;
		$path = !empty( $_SERVER['REQUEST_URI'] ) ? esc_url($_SERVER['REQUEST_URI']) : '';
		...
		if ( $load_data ) {
			...
			echo '<input type="hidden" class="arlite_is_preview" value="' . intval( esc_attr( $arflite_preview ) ) . '" />';

			if ( $arflite_preview == true ) {
				echo '<input type="hidden" class="arflite_preview_form" value="' . esc_attr( base64_encode( json_encode( $arflite_forms_loaded[0] ) ) ) . '" />';
			}
			...
			echo '<input type="hidden" class="arflite_script_url" value="' . esc_url( ARFLITESCRIPTURL ) . '" />';

			echo '<input type="hidden" class="arflite_file_error" value="' . esc_html__( 'Sorry, this file type is not permitted for security reasons', 'arforms-form-builder' ) . '" />';

			echo '<input type="hidden" class="arflite_no_file_selected" value="' . esc_html__( 'No File Selected', 'arforms-form-builder' ) . '" />';

			echo '<input type="hidden" class="arflite_blank_field_message" value="' . esc_html__( 'This field cannot be blank', 'arforms-form-builder' ) . '" />';
			...
			if ( $force_load ) {
				echo '<script defer id="arflite_temp_script" type="text/javascript">';
					echo 'var arflite_force_load = (function($){
                        var ARFormsliteApp = {
                            init:function(){
                                ...
                                setTimeout(function() {
									...
									echo $arflite_front_footer_script; //phpcs:ignore
                                    echo 'arflite_initialize_control_js();
									if( is_elementor != 1 ){'; 
										if ( ( isset( $_SERVER['HTTP_X_REQUESTED_WITH'] ) && $_SERVER['HTTP_X_REQUESTED_WITH'] == 'XMLHttpRequest' ) ) {
											echo 'arflite_initialize_form_control_onready();';
											echo 'arflite_init_bootstrap_validation();';
										}
								echo '}}, 500);
                            }';
						echo '};
	...
	function arflitedestroy() {
		...
		if ( ! current_user_can( 'arfdeleteentries' ) ) {
			...
			wp_die( $arflitesettings->admin_permission ); //phpcs:ignore
		}
		...
		if ( $params['action'] == 'create' && isset( $arflitecreatedentry[ $form->id ] ) && isset( $arflitecreatedentry[ $form->id ]['entry_id'] ) && is_numeric( $arflitecreatedentry[ $form->id ]['entry_id'] ) ) {
			...
			if ( $conf_method == 'redirect' ) {
				...
				if ( $success_url == false ) {
					...
					$return['message']     = $message;
					$return                = apply_filters( 'arflite_reset_built_in_captcha', $return, $_POST ); //phpcs:ignore 
					if ( $arflitesettings->form_submit_type == 1 ) { //phpcs:ignore
						echo json_encode( $return );
						exit;
					}
	...
	function arflitedisplay_list( $params = false, $message = '', $page_params_ov = false, $current_page_ov = false, $arflite_errors = array() ) {
		...
		if ( $form ) {
			...
			$form_options = $wpdb->get_row( $wpdb->prepare( 'SELECT `form_css`,`options` FROM `' . $ARFLiteMdlDb->forms . '` WHERE `id` = %d', (int) $form->id ) ); //phpcs:ignore
		...
		foreach ( $search_str as $search_param ) {
			...
			if ( empty( $fid ) || is_numeric( $fid ) ) {
				...
				if ( $data_fields = $arfliteform->arflite_has_field( 'data', $form_id, false ) ) {
					...
					$data_form_ids = $wpdb->get_col( "SELECT form_id FROM $ARFLiteMdlDb->fields WHERE id in (" . implode( ',', $df_form_ids ) . ')' ); //phpcs:ignore
	...
	function arflite_retrieve_form_entry_data() {
		...
		$form_select = $wpdb->get_row( $wpdb->prepare( 'SELECT * FROM `' . $ARFLiteMdlDb->forms . '` WHERE `id` = %d AND `is_template` != %d AND `status` = %s', $form_id, 1, 'published' ) ); //phpcs:ignore
		...
		$total_records = $wpdb->get_var( 'SELECT count(*) as total_entries FROM `' . $ARFLiteMdlDb->entries . '` it WHERE ' . $where_clause ); //phpcs:ignore
		...
		$columns_list_res = $wpdb->get_results( $wpdb->prepare( 'SELECT columns_list FROM ' . $ARFLiteMdlDb->forms . ' WHERE id = %d', $form_id ), ARRAY_A ); //phpcs:ignore
		...
		if ( count( $items ) > 0 ) {
			...
			foreach ( $items as $key => $item ) {
				...
				$arfliteget_url = $wpdb->get_row( $wpdb->prepare( 'SELECT entry_value FROM ' . $ARFLiteMdlDb->entry_metas . " WHERE field_id='%d' AND entry_id='%d'", '-' . 0, $item->id ) ); //phpcs:ignore
				if ( ! empty( $arfliteget_url ) ) {
					...
					if ( ! empty( $http_referrer_url ) ) {
						...
						$data[ $ai ][ $ni + 5 ] = isset( $http_page_url[1] ) ? urldecode( $http_page_url[1] ) : '';
					}
				} else {
	...
	function arflite_get_entries_list_edit( $id = '', $arffieldorder = array(), $arfinnerfieldorder = array() ) {
		...
		global $arflite_db_record, $arflitefield, $arfliterecordmeta, $user_ID, $arflitemainhelper, $arfliterecordhelper, $wpdb, $ARFLiteMdlDb;
		...
		$arfliteget_url = $wpdb->get_row( $wpdb->prepare( 'SELECT entry_value FROM ' . $ARFLiteMdlDb->entry_metas . " WHERE field_id='%d' AND entry_id='%d'", '-' . 0, $id ) ); //phpcs:ignore
		if ( ! empty( $arfliteget_url ) ) {
			$http_referrer_url = $arfliteget_url->entry_value;
			if ( ! empty( $http_referrer_url ) ) {
				$http_page_url    = explode( '|', $http_referrer_url );
		...
		foreach ( $fields as $field ) {
			...
			if ( $field->type == 'captcha' && isset( $_POST['recaptcha_challenge_field'] ) ) { //phpcs:ignore
				...
				if ( $site_key == '' || $private_key == '' ) {
					...
					$arflite_errors[ $field_id ] = ( ! isset( $field->field_options['invalid'] ) || $field->field_options['invalid'] == '' ) ? $arflitesettings->re_msg : $field->field_options['invalid'];
				} else {
	...
	function arflite_internal_check_recaptcha() {
		...
		global $wpdb, $arflite_errors, $arflitefieldhelper, $arflitemaincontroller,$arflitesettings;
