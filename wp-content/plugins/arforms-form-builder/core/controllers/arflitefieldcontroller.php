...
class arflitefieldcontroller {
	...
	function arflite_input_html( $field, $echo = true ) {
		...
		global $arflitesettings, $arflitenovalidate;
	...
	function arflite_prevalidateform_outside() {
		...
		if ( isset( $_POST['token'] ) && '' != $_POST['token'] && ! wp_verify_nonce( sanitize_text_field( $_POST['token'] ), 'arflite_validate_outside_nonce' ) ) {
			echo 0;
			die;
		}
	...
	function arflite_resetformoutside() {
		...
		if ( isset( $_POST['token'] ) && '' != $_POST['token'] && ! wp_verify_nonce( sanitize_text_field( $_POST['token'] ), 'arflite_reset_form_outside_nonce' ) ) {
			$returnarr                = array();
